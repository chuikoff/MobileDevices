...
BOOL usys()
{
	return TRUE;  // Win9x is gone; always use the Unicode NT APIs
}
...
void copyfinddatawa(WIN32_FIND_DATA *lpFindFileDataA,WIN32_FIND_DATAW *lpFindFileDataW)
{
	...
	walcopy(lpFindFileDataA->cFileName,lpFindFileDataW->cFileName,countof(lpFindFileDataA->cFileName)-1);
	walcopy(lpFindFileDataA->cAlternateFileName,lpFindFileDataW->cAlternateFileName,countof(lpFindFileDataA->cAlternateFileName)-1);
}
...
WCHAR* wcslcatbackslash(WCHAR* thedir,int maxlen)
{
	if (thedir[0] && wcslen(thedir)<(DWORD)maxlen)
		if (thedir[wcslen(thedir)-1]!='\\')
			wcscat_s(thedir,maxlen,L"\\");
	return thedir;
}

void wcutlastbackslash(WCHAR* thedir)
{
	if (thedir[0]) {
		WCHAR* p=thedir+wcslen(thedir)-1;
		if (p[0]=='\\')
			p[0]=0;
	}
}

LPWSTR wstrnew(LPWSTR str)
{
	LPWSTR retval=NULL;
	int len=0;
	if (str) {
		len=(int)wcslen(str)+1;
		retval=(LPWSTR)CoTaskMemRealloc(NULL,len*sizeof(WCHAR));
	}
	if (retval)
		wcscpy_s(retval,len,str);
	return retval;
}
